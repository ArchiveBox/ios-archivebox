...
final class LocalServer {
    static let address = "http://api.archivebox.localhost:18080"
    ...
    func launch(settings: SettingsModel) {
        ...
        task = Task {
            ...
            do {
                #if !arch(arm64)
                throw EngineFailure("ArchiveBox Server requires an Apple silicon Mac. You can still connect to a remote server.")
                #endif
                ...
                guard let app, Bundle(url: app)?.bundleIdentifier == "io.archivebox.Server" else {
                    throw EngineFailure("The installed app is not ArchiveBox Server.")
                }
                ...
                throw EngineFailure("The local server is not ready. Open ArchiveBox Server’s menu-bar Settings for startup details.\n\(failure)")
            } catch {
    ...
    private func download() async throws -> URL {
        ...
        guard (response as? HTTPURLResponse)?.statusCode == 200,
              ...
              let asset = release.assets.first(where: { $0.name == "ArchiveBox-Server-arm64.zip" }),
            ...
            throw EngineFailure("No verified ArchiveBox Server release is published yet. Install the companion manually, or try again after a release is available.")
        }
        guard asset.browser_download_url.scheme == "https", asset.browser_download_url.host == "github.com",
            ...
            throw EngineFailure("Unexpected companion download location.")
        }
        ...
        guard (downloadResponse as? HTTPURLResponse)?.statusCode == 200 else { throw EngineFailure("The companion download failed.") }
        ...
        let hash = try await EngineCommand.run(URL(fileURLWithPath: "/usr/bin/shasum"), ["-a", "256", zip.path])
        guard hash.status == 0, hash.output.split(whereSeparator: \.isWhitespace).first?.lowercased() == digest.dropFirst(7).lowercased() else { throw EngineFailure("The companion download checksum did not match.") }
        let expanded = staging.appending(path: "expanded")
        let extraction = try await EngineCommand.run(URL(fileURLWithPath: "/usr/bin/ditto"), ["-x", "-k", zip.path, expanded.path])
        guard extraction.status == 0 else { throw EngineFailure(extraction.output) }
        let app = expanded.appending(path: "ArchiveBox Server.app")
        guard Bundle(url: app)?.bundleIdentifier == "io.archivebox.Server" else { throw EngineFailure("The download is not ArchiveBox Server.") }
        let signature = try await EngineCommand.run(URL(fileURLWithPath: "/usr/bin/codesign"), ["--verify", "--deep", "--strict", "-R=anchor apple generic and certificate leaf[subject.OU] = Q3VA4FKRSA", app.path])
        guard signature.status == 0 else { throw EngineFailure("The companion does not have ArchiveBox’s expected signing identity.") }
        let assessment = try await EngineCommand.run(URL(fileURLWithPath: "/usr/sbin/spctl"), ["--assess", "--type", "execute", app.path])
        guard assessment.status == 0 else { throw EngineFailure("macOS did not approve this companion. The release must be signed and notarized.") }
        try Task.checkCancellation()
