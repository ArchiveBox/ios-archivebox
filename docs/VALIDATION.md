...
- Launched the companion using the main client's **This Mac → Run server locally** button. The main client discovered `http://api.archivebox.localhost:5797`; the real server returned its OpenAPI document with HTTP 200, version 0.9.35rc449.
...
## HTTP settings and toolbar URL (2026-09-17)

- Added selectable/clickable BASE_URL text in the toolbar and an HTTP, TLS, and DNS section with server-provided security-mode choices and Apply & Restart. The configured `auto` value is read separately from ArchiveBox's derived effective mode.
- Live HTTPSettingsAcceptance passed: invalid URL left container creation time unchanged; a changed localhost hostname and single-domain mode were persisted, the container was recreated without BASE_URL/security-mode environment overrides, and the new origin returned HTTP 200. Original BASE_URL and `auto` were restored through a second restart. Existing users remained unchanged; SQLite quick_check was ok and the existing snapshot remained present.
- The fresh-collection init/config command succeeded in a separate real transient container. Configuration is stored in the mounted collection rather than an immutable container environment. Readiness uses the internal loopback health endpoint so external DNS/TLS does not control container startup detection.
- Release compilation and deep/strict signature verification passed. Native toolbar click/selection and visual form verification remain subject to the previously recorded desktop automation helper failure.

## Menu and browser setup (2026-09-17)

- Native server menu now separates disabled status/metrics from browser/client/settings shortcuts and pause/quit controls. Menu sampling is on-open, throttled to 30 seconds, with no background menu timer.
- `ServerApp/Tests/ArchivingAcceptance.swift` passed against the real installed companion: CLI-created active crawl paused and resumed, sealed crawl remained sealed, test crawls removed. The initial check exposed that this image's `crawl create --status sealed` does not preserve that state; acceptance uses the public `crawl update --status sealed` command instead.
- Companion release packaging and deep/strict signature verification passed. Updated bundle installed at `~/Applications/ArchiveBox Server.app`. Desktop visual checks remain blocked by `Sky Computer Use native pipe closed before response`.
- Browser Extension settings includes Safari settings, Chrome/Brave's official Chrome Web Store listing, Firefox Add-ons, and GitHub source. Safari uses public native settings APIs (iOS 26.2+; earlier versions show instructions). Font Awesome brand assets use template rendering.
- Generic iOS Simulator, macOS, and signed direct macOS builds passed, including final SVG assets. No simulators were downloaded. Store navigation and native button clicks were not UI-verified because of the desktop helper failure.

## Four-screen client and automatic checks (2026-09-17)

- Sidebar/tab order is Add URLs, Archive (snapshots grid), Admin, Connection Settings; Connection Settings remains the initial screen. Local/remote profiles use mutually exclusive segmented controls on macOS.
- Server and API-key inputs use cancellable 600 ms debounced validation. The macOS UI was checked against an unreachable localhost port and a real server: red failure/disabled shortcuts, corrected API hostname, green success/enabled shortcuts. URL edits clear the draft key before probing another destination.
- Real iPhone Simulator test `testAutomaticConnectionAndScreens` passed against the installed server, including invalid server/key checks, enabled Admin/Get Key without a valid key, Get Key selecting embedded Admin, and the three web screens. A first run exposed the keyboard covering the tab bar; Return/scroll dismissal was added and the rerun passed. Final route test result: `/tmp/ios-archivebox-tools/admin-route.xcresult`.
...
- Navbar follow-up: load Django responsive.css immediately after base.css, before ArchiveBox’s navbar overrides, and prevent flex shrinking of the header. Real authenticated WebKit checks at 320/480/760/814/1000px confirmed every visible navbar link, button and image stays within the header’s vertical bounds, with no horizontal document overflow. Signed Mac build passed (`/tmp/ios-archivebox-tools/navbar-fit.log`).


## Sectioned sidebar and optional Safari connection (2026-09-17)

- Added the requested Collection, Admin, and Help destinations, plus Personas. Add URLs stays first and native Connection Settings last/default. All server routes were resolved against Django in the running image. Pages are allocated on first visit and retained independently with a shared WebKit cookie store.
- Signed Mac build and five Swift tests passed. Real iPhone `testAutomaticConnectionAndScreens` passed (`sidebar-phone3.xcresult`), including the clickable Admin heading. It exposed a compact-navigation issue fixed by using a real NavigationLink for that heading.
- Safari now accepts its own server/key pair or automatically uses the app pair when no extension server is configured. Missing app configuration no longer blocks options. Editing a manual server clears its key. The only Apple-specific JavaScript is the small optional native bridge and checked packaging patch; generated WXT output stays ignored. Shared upstream commit a3929b6 opens settings through the standard runtime API.
