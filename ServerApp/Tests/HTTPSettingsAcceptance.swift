...
@main struct HTTPSettingsAcceptance {
    static func check(_ condition: Bool, line: Int = #line) throws {
        if !condition { throw CommandFailure(message: "HTTP settings assertion failed at line \(line)") }
    }
    static func main() async throws {
        guard CommandLine.arguments.count == 2 else { throw CommandFailure(message: "Pass ArchiveBox Server.app's path") }
        ...
        } catch let error as CommandFailure { try check(error.message.contains("server origin")) }
        ...
        do {
            ...
            print("PASS: invalid input leaves container unchanged; config saved; container recreated; new host responds; original settings restored; users preserved")
        } catch {
