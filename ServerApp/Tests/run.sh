...
cd "$(dirname "$0")/../.."
case "${1:-}" in
    Management|HTTPSettings|Archiving|OpenCode) check="$1" ;;
    *) echo 'Usage: ServerApp/Tests/run.sh Management|HTTPSettings|Archiving|OpenCode "/path/to/ArchiveBox Server.app"' >&2; exit 2 ;;
esac
